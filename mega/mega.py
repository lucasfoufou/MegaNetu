...
class Mega(object):
    ...
    def download_url(self, url, dest_path=None, dest_filename=None):
        ...
        file_id = path[0]
        file_key = path[1]
        return self._download_file(file_id, file_key, dest_path, dest_filename, is_public=True)
    ...
    def _download_file(self, file_handle, file_key, dest_path=None, dest_filename=None, is_public=False, file=None):
        ...
        if dest_filename is None:
            ...
            extension = attribs['n'].split('.')
            shutil.move(temp_output_file.name, dest_path + file_name + "." + "".join(extension[-1:]))
            return dest_path + file_name + "." + "".join(extension[-1:])
    ...
    def upload(self, filename, dest=None, dest_filename=None):
        ...
        if file_size > 0:
            for chunk_start, chunk_size in get_chunks(file_size):
                print("{:.2f}".format(chunk_start*100.0/file_size) + "%")
                chunk = input_file.read(chunk_size)
